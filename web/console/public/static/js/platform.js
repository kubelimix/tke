...
define('config/config', function (require, exports, module) {
  ...
  let config = {
    ...
    //无需上报的错误码
    ignoreErrCode: [
      4, 7, 9, 12, 28, 29, 40, 41, 42, 43, 230, 231, 309, 506, 509, 510, 808, 819, 833, 834, 842, 900, 901, 902, 903,
    ...
    beforeNavigate: function (originalUrl, doNavigate) {
      let _beforeNavigate = function () {
        ...
        // 国际版路由检查（部分项目不经过 beforeContentRender）
        if (appUtil.isI18n()) {
  ...
  var startupRouter = function () {
    ...
    let setupIframeBusinessLoader = function (businessKey) {
      ...
      config['iframeRouteCopy'][routeKey] = config['extendRoutes'][routeKey] = function () {
        let iframeRouteUrl = iframeRouteConfig.map[businessKey].content;

        // 如果base url以#结尾，说明项目期望spa
        ...
        let loadIFrameView = function () {
          pageManager.loadView('iframe', null, [
            businessKey,
            url,
            {
      ...
      (function (key) {
        ...
        config['extendRoutes']['/' + name + jsModuleRouteConfig.suffixRule] = function () {
          ...
          let islteIe8 =
            ...
            /MSIE 8.|MSIE 7.|MSIE 6./i.test(navigator.appVersion);
          let islteIe9 =
            navigator.appName === 'Microsoft Internet Explorer' &&
            /MSIE 9.|MSIE 8.|MSIE 7.|MSIE 6./i.test(navigator.appVersion);
          ...
          appUtil.parallel(
            ...
            function () {
              ...
              if (shouldGoOn) {
                ...
                pageManager.cleanupBeforeNextRender =
                  (isExternalSource && !isBeeRouteMode && !isReactApp) || (lstController && lstController != name);
                pageManager.loadCommon.apply(pageManager, para);
              }
            }
          );
...
define('config/iframeRouter', function (require, exports, module) {
  ...
  for (let i = 0; i < versions.length; i++) {
    ...
    if (intlSites[version] && version != 'intl') {
      g_config_data['iframeRouter_' + version] = $.extend(
        {},
        g_config_data['iframeRouter_intl'],
        g_config_data['iframeRouter_' + version]
      );
    }
...
define('config/jsRouter', function (require, exports, module) {
  ...
  for (let i = 0; i < versions.length; i++) {
    ...
    if (intlVersions[version] && version != 'intl') {
      g_config_data['jsRouter_' + version] = $.extend(
        {},
        g_config_data['jsRouter_intl'],
        g_config_data['jsRouter_' + version]
      );
    }
...
define('config/submenu', function (require, exports, module) {
  ...
  for (let i = 0; i < versions.length; i++) {
    ...
    if (intlVersions[version] && version != 'intl') {
      g_config_data['submenu_' + version] = $.extend(
        {},
        g_config_data['submenu_intl'],
        g_config_data['submenu_' + version]
      );
    }
...
define('lib/Class', function (require, exports, module) {
  ...
  module.exports = {
    ...
    extend: function (protoProps, staticProps) {
      ...
      let constructor = protoProps.hasOwnProperty('constructor')
...
define('lib/appUtil', function (require, exports, module) {
  ...
  let projectId;
  let managePerm;
  let verified, verifiedOverseas, _isI18n;
  let comData;
  let defaultPid = -1;
  ...
  module.exports = {
      ...
      if (Array.isArray(obj)) {
        ...
        for (let prop in obj) {
          if (obj.hasOwnProperty(prop)) {
            let newProp = prop;
            if (!notChangeAllUppercaseFeild || (notChangeAllUppercaseFeild && !/^[A-Z]+$/g.test(prop))) {
              let firstLetter = prop.substr(0, 1);
              newProp = (type === 'upper' ? firstLetter.toUpperCase() : firstLetter.toLowerCase()) + prop.slice(1);
            }
            if (newProp !== prop) {
            ...
            if (isObject(obj[newProp]) || Array.isArray(obj[newProp])) {
              obj[newProp] = _self.changeLetterCase(obj[newProp], type, notChangeAllUppercaseFeild);
            }
          }
        }
      }
    ...
    /**
     * 获取区域Id. 项目代码直接使用该方法, 可能会造成请求的地域混乱, 应避免使用
     * @method getRegionId
     * @return {Int} regionId
     */
     ...
     */
    size: function (str) {
      return str.replace(/[^\u0000-\u00FF]/gim, '**').length;
    },
  ...
  var checker = {
    ...
    domPath: function (elem, options) {
      ...
      let getDomPath = function (el) {
        let elString, elId, elClass, elIdClass;
      ...
      $this.each(function () {
        let current, domPathItem, pathArray;
    ...
    check: function (container) {
      ...
      if (pathObj && notTranslateNodes && notTranslateNodes.length && manager.reportTranslate) {
        ...
        if (self.lastParams && self.lastParams === JSON.stringify(params)) {
          // 忽略掉重复内容上报
          return;
        }
...
define('widget/devtoolsDetector/devtoolsDetector', function (require, exports, module) {
  ...
  $(window)
    .off('resize.devtoolsDetector')
    .on(
      'resize.devtoolsDetector',
      appUtil.throttle(function () {
        check();
      }, 500)
    );
...
define('nmc/config/config', function (require, exports, module) {
  ...
  let config = {
    ...
    /**
         ...
         */
    html404: '<div id="tt404" class="error-page">' + ' <h2>404</h2> <p>您访问的页面没有找到！</p></div>',
    ...
    /**
         ...
         */
    htmlOops:
      '<div id="ttOops" class="error-page">' +
  ...
  let util = {
    cookie: {
      ...
      setSecure: function (name, value, domain, path, hour) {
        ...
        document.cookie =
          ...
          (hour ? 'expires=' + expire.toGMTString() + '; ' : '') +
          (path ? 'path=' + path + '; ' : 'path=/; ') +
          (domain ? 'domain=' + domain + '; secure' : 'domain=' + document.domain + '; secure');
    ...
    tmpl: (function () {
      ...
      return function tmpl(str, data, mixinTmpl) {
        ...
        var fn = strIsKey
          ? (_cache[str] = _cache[str] || tmpl(_getTmplStr(document.getElementById(str).innerHTML, mixinTmpl)))
          : new Function(
              'obj',
              '_escape',
              "var _p='';with(obj){_p+='" +
    ...
    evt.addCommonEvent('click', {
      nav: function (e) {
        ...
        if (topSubMenuTrigger.length) {
          setTimeout(function () {
            $('#topnav')
              .find('[data-event="top_submenu"]')
              .removeClass('qc-nav-hover qc-header-panel-border qc-nav-select');
  ...
  let pageManager = {
    ...
    getSidebarRenderRouter: function () {
      ...
      if (prevSidebarRenderRouter == fragment && self.sidebar.find('.qc-menu-fold').length) {
        ...
        if (
          prevSidebarRenderRouter &&
          nmcConfig.subMenu[fragment] &&
          nmcConfig.subMenu[fragment].relate == prevSidebarRenderRouter
        ) {
          return false;
        }
        return fragment;
    ...
    renderView: function (obj, params) {
      ...
      //渲染内容前执行项目逻辑
      let beforeContentRender = obj.beforeContentRender
        ? obj.beforeContentRender.bind(obj)
        : nmcConfig.beforeContentRender
        ? nmcConfig.beforeContentRender.bind(nmcConfig)
        : null;
    ...
    isNoRouteMatch: function (controller) {
      ...
      // 命中了内部路由但是又不是内部项目的，认为无路由匹配，可以渲染 404
  ...
  let router = {
    ...
    navigate: function (url, silent, replacement) {
      ...
      this.option.beforeNavigate(url, function () {
        if (url !== _self.fragment && !silent) {
          //silent为true时，只路由不改变地址栏
          if (_self.debug) {
  ...
  let tips = {
    ...
    showFlash: function (str, icoClassName) {
      ...
      flashMsg
        .html(str)
        .show()
        .attr('class', icoClassName + ' fade-in');
    },
    ...
    //手动的请求计数, 用于接入项目jsonp方式等复用loading逻辑
